...
function MyJobs() {
  ...
  useEffect(() => {
    setIsLoading(true)
    fetch(`http://localhost:8000/my-jobs/aa@gmail.com`).then(res => res.json()).then(data => {
      setJobs(data)
      setIsLoading(false)
    })
  }, [])
  ...
  const handleSearch = () => {
    const filteredJobs = jobs.filter((job) => job.jobTitle.toLowerCase().indexOf(searchText.toLowerCase()) !== -1)
    ...
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
      <div className="my-jobs-container">
        ...
        <div className='search-box p-2 text-center mb-2'>
          <input type="text" name="search" id="search" className='py-2 pl-3 border focus:outline-none lg:w-6/12 mb-6 w-full' onChange={(e) => setSearchText(e.target.value)} />
          <button className='bg-blue text-white font-semibold px-8 py-2 rounded-sm mb-4' onClick={handleSearch}>Search</button>
        </div>
      </div>

      {/* table */}
      <section className="py-1 bg-blueGray-50 w-full mb-10">
        <div className="w-full xl:w-8/12 mb-12 xl:mb-0 px-3 mx-auto mt-5">
        ...
        {/* pagination */}
        <div className="flex justify-center text-black space-x-5">
          {
            currentPage > 1 && (
